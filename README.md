...
> **Cinematic Minimalism, strict dark mode.** Built as a senior frontend engineer would:
> clean architecture, reusable typed components, a swappable mock-data layer, and smooth
> "luminance + scale" motion.
...
- **Framer Motion** for hover / scale / transition animation
...
│   ├── App.tsx          # ProfileProvider + scroll-to-top + <Outlet/>
│   ├── router.tsx       # all route definitions
│   └── providers/       # ProfileProvider (active profile state)
├── components/
...
│                        #   videos, photos, collection, recently-added,
│                        #   search, upload) — each a barrel-exported page
...
| `/login`          | Sign In                                           |
| `/signup`         | Sign Up                                            |
| `/profiles`       | "Who's watching?" profile selection               |
...
| ----------------------- | ------------ |
| **Memory photos/videos** (cards, hero, timeline) — title, description, date, location, image, hearts | [`src/data/memories.mock.ts`](src/data/memories.mock.ts) |
